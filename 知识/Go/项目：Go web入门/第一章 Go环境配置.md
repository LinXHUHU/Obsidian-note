...
#Go语言特点 
- 编译快
- 依赖分析更容易， 避免使用include 管理麻烦
- 静态语言， 执行速度快
- 有垃圾回收，失去一部分性能，换取的，很值得


# 1.1 安装GO

**多版本GO管理工具**：GVM（以后可以尝试使用）
...
解压到 /usr/local  (：`/usr/local` 通常包含您从源代码编译或手动安装的软件。这样可以避免与系统自带软件包的冲突，以及随着系统更新而被覆盖。)
...
设置默认的go版本  也可以使用下面的命令，省去每次调用gvm use的麻烦： gvm use go1.14.7 --default
